...
const ReceiptDecision = ({ isApproved, reasons, receiptId, onGoBack }: ReceiptDecisionProps) => {
  ...
  if (isAppealing) {
    return (
      <View
        className="w-full bg-surface-container-low rounded-[24px] p-6 border mb-6"
        style={{
      ...
      >
        ...
        <View className="flex-row justify-between mt-4">
          ...
          <TouchableOpacity
            ...
            disabled={isSubmitting || appealText.trim().length === 0}
          ...
          >
            <LinearGradient
              colors={appealText.trim().length > 0 ? ['#630ED4', '#7C3AED'] : ['#E6E0E9', '#E6E0E9']}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <MaterialIcons name="send" size={20} color={appealText.trim().length > 0 ? "white" : "#7b7487"} />
                  <Text className={`font-headline font-bold text-base ${appealText.trim().length > 0 ? 'text-white' : 'text-on-surface-variant'}`}>
                    Submit
                  </Text>
                </>
              )}
